...
Pour utiliser MQTT, placez d'abord **se connecter au Wi-Fi**, puis **se connecter au broker MQTT**. Indiquez l'adresse du broker, son port et, si nécessaire, les identifiants. Ajoutez ensuite **s'abonner au topic MQTT**, saisissez le topic à écouter et choisissez une livraison standard ou confirmée. Le bloc de départ **quand un message MQTT est reçu** permet de filtrer le topic, de ranger le contenu reçu dans une variable Texte et de déclencher les blocs placés dessous. La bibliothèque MQTT est incluse automatiquement : aucune installation supplémentaire n'est demandée.
